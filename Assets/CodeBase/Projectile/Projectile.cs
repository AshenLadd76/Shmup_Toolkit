...
namespace CodeBase.Projectile
{
    ...
    public class NeoProjectile : IProjectile, IPoolable<NeoProjectile>, ICollisionObject
    {
        ...
        public NeoProjectile( Transform projectileTransform )
        {
            _transform = projectileTransform ?? throw new ArgumentNullException(nameof(projectileTransform));

            _spriteRenderer = _transform.GetComponent<SpriteRenderer>() ?? throw new MissingComponentException(
                                               $"NeoProjectile requires a SpriteRenderer on '{_transform.name}'.");
